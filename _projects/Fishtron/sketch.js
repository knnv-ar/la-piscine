...
function draw() {
  ...
  if (keyIsPressed && key == "4") {
    ...
    let billetes = frameCount % 512; // Movimiento horizontal
    push();
...
/*Fondo ciudad
  ...
  for (let y = 170; y < 292; y += paso / 3) {
    for (let x = 150; x < 180; x += paso / 2) {
      square(x, y, 1);
    }
  }
  for (let y = 250; y < 288; y += paso / 3) {
    for (let x = 184; x < 210; x += paso / 2) {
      square(x, y, 1);
    }
  }
  for (let y = 200; y < 284; y += paso / 3) {
    for (let x = 265; x < 271; x += paso / 2) {
      square(x, y, 1);
    }
  }
*/
  ...
  pop();
  /* 
 Corazon/ALMA interactiva/animada
  
  fill(255);
  stroke(255);
  ellipse(404,238,8,15);
  */
...
/*
-Hacer que aparezca un texto en la consola cuando apretas 4.
-Que el público fluctue.
