...
let stepSize = 0.015;
let fadeTime = 5; // Time in seconds before a letter starts fading
...
function draw() {
  ...
  // Draw curves for each active letter
  let letters = 'abcdefghijklmnopqrstuvwxyz';
  for (let i = 0; i < letters.length; i++) {
    let letter = letters[i];
    let t = eval('t_' + letter);
    ...
    if (t >= 0) {
      ...
      // ... (add else if blocks for other letters)
