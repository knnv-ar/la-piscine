/*
(María) Serena Maurenza - AM 1 - Cátedra Lacabanne
Oráculo-19
*/
...
function preload() {
  syne = loadFont('assets/syne-tactile-regular.ttf');
...
function setup() {
  ...
  //Acá creo la gramática con la data del JSON
...
function draw() {
  ...
  //Noise flow field de fondo con partículas
  ...
  //Animación y líneas circundantes al ojo
  
  const t = millis() / 1000;
  const d = 5;
  const p = (t / d) % 1;
  let anim = sin((p * PI) / 1.5) * cos((p * PI) / 2) * 0.5 + 0.5;
  let radius = 300 * anim;

  let circleResolution = 30;
  ...
  bezier(-100, 0, -50, -50, 50, -50, 100, 0);
  bezier(-100, 0, 10, 90,  100, 0, 100, 0);
  fill(230);
  circle(0, 0, 15);
  noFill();
  strokeWeight(3);
  ...
  strokeWeight(5);
  noStroke();
  let oracle = text('el oráculo-19', width/2, 750);
...
//Función que pinta el texto generado

function generateText() {
  fill(255, 255, 255, opacity);
  text(expansion, mouseX, mouseY, 700);
  opacity += 1;
}

//Función que genera un texto nuevo y maneja el switch para mostrar la predicción

function mouseClicked() {
  expansion = grammar.flatten('#origin#');
  predic = !predic;
}


// === Fusión de archivo: particle.js ===
function Particle() {
